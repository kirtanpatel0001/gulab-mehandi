...
export default function AdminAppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    setLoading(true);
    // Fetching from your 'bookings' table
    ...
    <div className="max-w-6xl mx-auto pb-12">
      ...
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="bg-white border border-[#1B342B]/10 px-6 py-3 rounded-full shadow-sm">
          <p className="text-[10px] uppercase tracking-widest font-bold text-[#A67C52]">Pending Requests: <span className="text-[#1B342B]">{appointments.filter(a => a.status === 'Pending' || !a.status).length}</span></p>
        </div>
      </header>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#FDFBF7] border-b border-[#1B342B]/5">
                <th className="px-8 py-6 text-[10px] uppercase tracking-widest text-[#1B342B]/50 font-bold">Client Details</th>
                <th className="px-8 py-6 text-[10px] uppercase tracking-widest text-[#1B342B]/50 font-bold">Service Type</th>
                <th className="px-8 py-6 text-[10px] uppercase tracking-widest text-[#1B342B]/50 font-bold">Date Requested</th>
                <th className="px-8 py-6 text-[10px] uppercase tracking-widest text-[#1B342B]/50 font-bold text-center">Status</th>
                <th className="px-8 py-6 text-[10px] uppercase tracking-widest text-[#1B342B]/50 font-bold text-right">Action</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={5} className="p-20 text-center text-[#A67C52] text-xs uppercase tracking-widest animate-pulse font-bold">Loading Appointments...</td>
                </tr>
              ) : appointments.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-20 text-center text-[#1B342B]/40 italic text-sm">No appointments found.</td>
                </tr>
              ) : (
                appointments.map((appt, index) => (
                  <tr key={appt.id} className={`border-b border-[#1B342B]/5 hover:bg-[#FDFBF7]/60 transition-colors duration-300 ${index === appointments.length - 1 ? 'border-none' : ''}`}>
                    ...
                    {/* CLIENT DETAILS (Mapped to first_name, last_name, email, phone) */}
                    <td className="px-8 py-6">
                      <div className="flex items-center mb-2">
                        <div className="w-8 h-8 rounded-full bg-[#1B342B]/5 flex items-center justify-center text-[#A67C52] font-serif text-sm mr-3 shadow-sm">
                          {appt.first_name ? appt.first_name.charAt(0).toUpperCase() : 'U'}
                        </div>
                        <p className="text-[#1B342B] text-sm font-bold">
                    ...
                    {/* SERVICE TYPE (Mapped directly from your db) */}
                    <td className="px-8 py-6 text-[#1B342B]/80 text-sm font-medium">
                      {appt.service_type || 'General Inquiry'}
                    </td>
