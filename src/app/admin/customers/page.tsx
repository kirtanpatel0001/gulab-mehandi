      ...
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="bg-white border border-[#1B342B]/10 px-6 py-3 rounded-full shadow-sm">
          <p className="text-[10px] uppercase tracking-widest font-bold text-[#A67C52]">
            Total Accounts: <span className="text-[#1B342B]">{customers.length}</span>
          </p>
        </div>
      </header>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#FDFBF7] border-b border-[#1B342B]/5">
                {['Account Holder', 'Contact Info', 'Access Level', 'System ID'].map((h, i) => (
                  <th key={h} className={`px-8 py-6 text-[10px] uppercase tracking-widest text-[#1B342B]/50 font-bold ${i === 3 ? 'text-right' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={4} className="p-20 text-center text-[#A67C52] text-xs uppercase tracking-widest animate-pulse font-bold">Loading Database...</td></tr>
              ) : customers.length === 0 ? (
                <tr><td colSpan={4} className="p-20 text-center text-[#1B342B]/40 italic text-sm">No accounts found.</td></tr>
                ...
                <tr key={customer.id} className={`border-b border-[#1B342B]/5 hover:bg-[#FDFBF7]/60 transition-colors duration-300 ${index === customers.length - 1 ? 'border-none' : ''}`}>
                  ...
                  <td className="px-8 py-6">
                    {customer.role === 'admin' ? (
                      <span className="px-3 py-1.5 text-[9px] uppercase tracking-[0.2em] font-bold rounded-full bg-purple-50 text-purple-700 border border-purple-200">Admin</span>
                    ) : (
                      <span className="px-3 py-1.5 text-[9px] uppercase tracking-[0.2em] font-bold rounded-full bg-gray-50 text-gray-600 border border-gray-200">Client User</span>
                    )}
                  </td>
