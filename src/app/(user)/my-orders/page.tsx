...
const detectUserCurrency = () => {
  try {
    ...
    if (tz.includes('Kolkata') || tz.includes('Calcutta') || tz.includes('Asia/Colombo')) return { code: 'INR', symbol: '₹', rate: 1 };
    if (tz.includes('London') || tz.includes('Europe')) return { code: 'GBP', symbol: '£', rate: 0.0095 };
    if (tz.includes('Dubai') || tz.includes('Asia/Dubai')) return { code: 'AED', symbol: 'د.إ', rate: 0.044 };
...
export default function MyOrdersPage() {
  ...
  const formatPrice = (basePrice: number) => `${currency.symbol}${(basePrice * currency.rate).toFixed(2)}`;
                  ...
                  <div className="p-6 md:p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
                    <div className="flex items-center space-x-5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#1B342B]/60 mt-1">
                    ...
                    <div className="w-full lg:w-1/2 mt-4 lg:mt-0 px-2 md:px-0">
                      <div className="flex items-center justify-between relative max-w-sm mx-auto lg:ml-auto lg:mr-0">
                        ...
                        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-[#A67C52] z-0 transition-all duration-1000" style={{ width: `${((currentStep - 1) / 3) * 100}%` }}></div>
                        ...
                        {['Ordered', 'Processing', 'Shipped', 'Delivered'].map((step, index) => {
                          ...
                          return (
                            <div key={step} className="relative z-10 flex flex-col items-center">
                              <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center font-bold text-[10px] md:text-xs border-2 transition-colors duration-500 ${isActive ? 'bg-[#A67C52] border-[#A67C52] text-white shadow-sm' : 'bg-white border-[#1B342B]/20 text-[#1B342B]/40'}`}>
                              ...
                              <p className={`absolute -bottom-6 text-[8px] md:text-[9px] uppercase tracking-widest font-bold whitespace-nowrap ${isActive ? 'text-[#1B342B]' : 'text-[#1B342B]/40'}`}>{step}</p>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <div className="px-6 py-4 bg-[#FDFBF7]/50 border-t border-[#1B342B]/5 flex justify-end">
                    ...
                    >
                      {isCurrentlyDownloading ? (
                        <><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div><span>Downloading...</span></>
                      ) : (
                        <><span>Download Receipt</span><svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg></>
                      )}
                    </button>
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #1B342B', paddingBottom: '40px', marginBottom: '40px' }}>
                <div style={{ width: '50%' }}>
                  ...
                  <div style={{ fontSize: '13px', color: '#444', lineHeight: '1.8' }}>
                    <div>123 Heritage Art Street, Vesu</div>
                    <div>Surat, Gujarat 395007, India</div>
                    <div style={{ marginTop: '8px' }}><strong>GSTIN:</strong> 24ABCDE1234F1Z5</div>
                  </div>
                </div>
                <div style={{ width: '50%', textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: '13px', color: '#444', lineHeight: '1.8' }}>
                    <div><strong>Invoice No:</strong> {selectedOrder.razorpay_order_id || selectedOrder.id.slice(0, 8)}</div>
                    <div><strong>Date:</strong> {new Date(selectedOrder.created_at).toLocaleDateString('en-IN')}</div>
                    <div><strong>Payment Status:</strong> {selectedOrder.status || 'PAID'}</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <div style={{ width: '65%', backgroundColor: '#f9fafb', padding: '32px', border: '1px solid #e5e7eb', borderRadius: '4px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', fontSize: '15px', color: '#444' }}>
                    <span>Subtotal</span><span>₹{(selectedOrder.total_amount / 1.18).toFixed(2)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', fontSize: '15px', color: '#444' }}>
                    <span>IGST (18%)</span><span>₹{(selectedOrder.total_amount - selectedOrder.total_amount / 1.18).toFixed(2)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px', paddingTop: '20px', borderTop: '2px solid #d1d5db', fontSize: '15px', fontWeight: 'bold', color: '#1B342B' }}>
                    <span>Grand Total</span><span>₹{selectedOrder.total_amount?.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
              </div>
            </div>
