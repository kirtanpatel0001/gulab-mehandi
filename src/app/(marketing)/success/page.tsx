...
function SuccessContent() {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      // ✅ No auth check needed — order ID from URL is the access key
      // Orders are readable by their ID from the success redirect
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] py-12 px-4 md:px-8 flex flex-col items-center relative overflow-hidden">
        ...
        <div className="flex justify-end mb-12">
          <div className="w-full md:w-1/2 bg-[#FDFBF7] p-6 border border-[#1B342B]/10 rounded-sm">
            ...
            {isINR && isLocalGujarat ? (
              <>
                <div className="flex justify-between text-sm text-[#1B342B]/80 mb-2"><span>CGST (9%)</span><span className="tabular-nums">{currencySymbol}{cgst.toFixed(2)}</span></div>
                <div className="flex justify-between text-sm text-[#1B342B]/80 mb-3"><span>SGST (9%)</span><span className="tabular-nums">{currencySymbol}{sgst.toFixed(2)}</span></div>
              </>
            ) : isINR ? (
              <div className="flex justify-between text-sm text-[#1B342B]/80 mb-3"><span>IGST (18%)</span><span className="tabular-nums">{currencySymbol}{totalTax.toFixed(2)}</span></div>
            ) : (
              <div className="flex justify-between text-sm text-[#1B342B]/80 mb-3"><span>Taxes</span><span className="tabular-nums">{currencySymbol}{totalTax.toFixed(2)}</span></div>
            )}
      ...
      <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 pb-12 w-full max-w-3xl justify-center">
        <button onClick={handleDownload} disabled={isDownloading} className="flex items-center justify-center space-x-2 bg-[#1B342B] text-[#FDFBF7] px-8 py-3.5 rounded-sm hover:bg-[#A67C52] transition-all duration-300 text-[11px] font-bold uppercase tracking-[0.2em] shadow-md disabled:opacity-70 min-w-[200px]">
          {isDownloading ? (
            <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></div>
          ) : (
            <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg><span>Download Invoice</span></>
          )}
        </button>
      ...
      <div className="absolute top-[-9999px] left-[-9999px]">
        <div id="invoice-pdf-template" className="bg-white p-14" style={{ width: '800px' }}>
          <div className="flex justify-between items-start border-b-2 border-[#1B342B] pb-8 mb-8">
            <div>
              ...
              <div className="text-xs text-[#1B342B]/80 leading-relaxed space-y-0.5">
                <p>123 Heritage Art Street, Vesu</p>
                <p>Surat, Gujarat 395007, India</p>
                <p className="mt-2"><span className="font-bold">Email:</span> hello@gulabmehndi.com</p>
                <p><span className="font-bold">Phone:</span> +91 98765 43210</p>
                <p className="mt-2"><span className="font-bold">GSTIN:</span> 24ABCDE1234F1Z5</p>
              </div>
            </div>
            <div className="text-right">
              <h1 className="text-3xl font-bold text-[#A67C52] uppercase tracking-[0.2em] mb-4">Tax Invoice</h1>
              <div className="text-xs text-[#1B342B]/80 space-y-1">
                <p><span className="font-bold">Invoice No:</span> {order?.razorpay_order_id}</p>
                <p><span className="font-bold">Date:</span> {formattedDate}</p>
                <p><span className="font-bold">Payment Status:</span> <span className="text-green-600 font-bold">PAID</span></p>
              </div>
            </div>
          </div>
          ...
          {address && (
            <div className="mb-10 w-1/2">
              ...
              <div className="text-[#1B342B]/80 text-sm leading-relaxed">
                ...
                <p>{address.city}, {address.state} {address.postal_code}</p>
                <p>{address.country}</p>
                <p className="mt-2"><span className="font-bold text-xs uppercase tracking-wider">Phone:</span> {address.phone}</p>
              </div>
            </div>
          )}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                <tr className="border-b border-[#1B342B]/10 text-sm">
                  ...
                  <td className="py-5 px-4 text-[#1B342B]/70 text-center text-xs">3304</td>
                  <td className="py-5 px-4 text-right tabular-nums font-medium">{currencySymbol}{subtotal.toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="flex justify-end mb-12">
            <div className="w-1/2 bg-[#FDFBF7] p-6 border border-[#1B342B]/10 rounded-sm">
              <div className="flex justify-between text-sm text-[#1B342B]/80 mb-3"><span>Subtotal (Exclusive of Tax)</span><span className="tabular-nums font-medium">{currencySymbol}{subtotal.toFixed(2)}</span></div>
              {isINR && isLocalGujarat ? (
                <><div className="flex justify-between text-sm text-[#1B342B]/80 mb-2"><span>CGST (9%)</span><span>{currencySymbol}{cgst.toFixed(2)}</span></div>
                <div className="flex justify-between text-sm text-[#1B342B]/80 mb-3"><span>SGST (9%)</span><span>{currencySymbol}{sgst.toFixed(2)}</span></div></>
              ) : isINR ? (
                <div className="flex justify-between text-sm text-[#1B342B]/80 mb-3"><span>IGST (18%)</span><span>{currencySymbol}{totalTax.toFixed(2)}</span></div>
              ) : (
                <div className="flex justify-between text-sm text-[#1B342B]/80 mb-3"><span>Taxes</span><span>{currencySymbol}{totalTax.toFixed(2)}</span></div>
              )}
              <div className="flex justify-between text-lg font-bold text-[#1B342B] pt-4 border-t border-[#1B342B]/20">
                <span>Grand Total</span>
                <span className="tabular-nums">{currencySymbol}{totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
