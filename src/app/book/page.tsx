...
export default function BookPage() {
  ...
  useEffect(() => {
    const fetchLocation = async () => {
      try {
        ...
        console.error("Could not auto-detect location, falling back to default.", error);
      }
    };
  ...
  // --- DYNAMIC PHONE LENGTH LOGIC ---
  // Returns the exact required length for your target markets, or a safe default for others
  const getPhoneConfig = (dialCode: string) => {
    switch (dialCode) {
      case '+1': return { min: 10, max: 10 }; // USA / Canada
      case '+91': return { min: 10, max: 10 }; // India
      case '+971': return { min: 9, max: 9 }; // UAE
      case '+61': return { min: 9, max: 9 }; // Australia
      case '+64': return { min: 8, max: 10 }; // New Zealand
      case '+65': return { min: 8, max: 8 }; // Singapore
      case '+60': return { min: 9, max: 10 }; // Malaysia
      case '+966': return { min: 9, max: 9 }; // Saudi Arabia
      case '+974': return { min: 8, max: 8 }; // Qatar
      case '+44': return { min: 10, max: 11 }; // UK
      default: return { min: 7, max: 15 }; // Global standard fallback
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (phoneNumber.length < phoneConfig.min) {
      alert(`Please enter a valid phone number. It looks a bit too short for ${selectedCountry.name}.`);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-6 lg:pt-4">
            ...
            <div className="flex bg-white border border-[#1B342B]/20 rounded-sm focus-within:border-[#A67C52] transition-colors relative" ref={dropdownRef}>
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 mt-1 w-[300px] bg-white border border-[#1B342B]/10 shadow-2xl rounded-sm z-50 overflow-hidden">
                  ...
                  <ul className="max-h-60 overflow-y-auto custom-scrollbar pb-2">
                    {/* --- TARGET MARKETS PINNED TO TOP --- */}
                    {!searchQuery && (
                        ...
                        {[
                          ...
                          <li 
                            ...
                            onClick={() => {
                              ...
                              setPhoneNumber(""); // Reset phone when country changes so lengths don't mismatch
                              setIsDropdownOpen(false);
                            }}
                    ...
                    {filteredCountries.length === 0 && (
                      <li className="px-4 py-4 text-sm text-[#1B342B]/50 text-center italic">
                        No countries found.
                      </li>
                    )}
                  </ul>
              ...
              <input 
                name="phone" 
                type="text" // Changed to text to handle the manual formatting
                required 
                value={phoneNumber}
          ...
          <div className="pt-4">
            <button 
              type="submit" 
              disabled={loading}
              className="border border-[#1B342B] text-[#1B342B] px-8 py-3 rounded-full hover:bg-[#1B342B] hover:text-[#FDFBF7] transition-colors duration-300 uppercase text-xs tracking-widest font-bold disabled:opacity-50"
            >
