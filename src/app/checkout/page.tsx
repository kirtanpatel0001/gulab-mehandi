...
export default function CheckoutPage() {
  ...
  const handlePayment = async () => {
    ...
    script.onload = async () => {
      try {
        // 2. Call our secure backend API to create the order
        ...
        const options = {
          ...
          handler: async function (response: any) {
            ...
            // 4. Verify the payment on the backend after the user pays
            ...
            if (verifyData.success) {
              // Redirect to our PDF Invoice Page using the actual Database Order ID
  ...
  if (loading) return <div className="min-h-screen flex justify-center items-center bg-[#FDFBF7]"><div className="w-8 h-8 border-2 border-[#1B342B]/20 border-t-[#A67C52] rounded-full animate-spin"></div></div>;
            ...
            <div className="bg-white p-6 md:p-8 rounded-sm shadow-sm border border-[#1B342B]/10">
              ...
              {!showForm ? (
                <div className="space-y-4">
                  ...
                  {/* ONLY show "Add Address" if they have less than 2! */}
                  {addresses.length < 2 && (
                    <button 
                      onClick={() => setShowForm(true)}
                      className="mt-4 border border-[#1B342B] text-[#1B342B] px-6 py-3 rounded-sm hover:bg-[#1B342B] hover:text-[#FDFBF7] transition-all duration-300 uppercase text-xs tracking-[0.2em] font-bold w-full md:w-auto"
                    >
                      + Add New Address
                    </button>
                  )}
                ...
                <form onSubmit={handleSaveAddress} className="space-y-4 animate-fade-in">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input required placeholder="Full Name" value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} className="border border-[#1B342B]/15 p-3 text-sm focus:ring-[#A67C52] outline-none rounded-sm" />
                    <input required type="tel" placeholder="Phone Number" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="border border-[#1B342B]/15 p-3 text-sm focus:ring-[#A67C52] outline-none rounded-sm" />
                  </div>
                  <input required placeholder="Street Address (House No, Building, Street)" value={formData.street_address} onChange={e => setFormData({...formData, street_address: e.target.value})} className="w-full border border-[#1B342B]/15 p-3 text-sm focus:ring-[#A67C52] outline-none rounded-sm" />
                  <div className="grid grid-cols-2 gap-4">
                    <input required placeholder="City" value={formData.city} onChange={e => setFormData({...formData, city: e.target.value})} className="border border-[#1B342B]/15 p-3 text-sm focus:ring-[#A67C52] outline-none rounded-sm" />
                    <input required placeholder="State" value={formData.state} onChange={e => setFormData({...formData, state: e.target.value})} className="border border-[#1B342B]/15 p-3 text-sm focus:ring-[#A67C52] outline-none rounded-sm" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <input required placeholder="PIN / Postal Code" value={formData.postal_code} onChange={e => setFormData({...formData, postal_code: e.target.value})} className="border border-[#1B342B]/15 p-3 text-sm focus:ring-[#A67C52] outline-none rounded-sm" />
                    <input required placeholder="Country" value={formData.country} onChange={e => setFormData({...formData, country: e.target.value})} className="border border-[#1B342B]/15 p-3 text-sm focus:ring-[#A67C52] outline-none rounded-sm" />
                  </div>
                  
                  <div className="flex space-x-4 pt-4 border-t border-[#1B342B]/10">
                    <button type="submit" className="bg-[#1B342B] text-white px-6 py-3 rounded-sm hover:bg-[#A67C52] text-xs font-bold uppercase tracking-widest transition-colors">
                      Save Address
                    </button>
                    <button type="button" onClick={() => { setShowForm(false); setEditingId(null); }} className="text-[#1B342B]/60 hover:text-[#1B342B] text-xs font-bold uppercase tracking-widest transition-colors">
                      Cancel
                    </button>
                  </div>
                </form>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-sm shadow-sm border border-[#1B342B]/10 sticky top-24">
              ...
              <div className="border-t border-[#1B342B]/10 pt-4 space-y-3 mb-6">
                <div className="flex justify-between text-sm text-[#1B342B]/80">
                  <span>Subtotal</span>
                ...
                <div className="flex justify-between text-lg font-bold text-[#1B342B] pt-4 border-t border-[#1B342B]/10 mt-4">
                  <span>Total</span>
              ...
              <button 
                ...
                className="w-full bg-[#1B342B] text-[#FDFBF7] py-4 rounded-sm hover:bg-[#A67C52] transition-colors duration-300 uppercase text-xs tracking-[0.2em] font-bold shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
              >
