        ...
        <div className="p-6 border-b border-[#1B342B]/10 flex justify-between items-center bg-white shrink-0">
          <h2 className="text-xl md:text-2xl font-serif text-[#1B342B] flex items-center tracking-wide">
            ...
            {isCartLoading && cartItems.length > 0 && (
              <span className="ml-3 w-3.5 h-3.5 border border-[#1B342B]/20 border-t-[#A67C52] rounded-full animate-spin"></span>
            )}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {!user ? (
            <div className="h-full p-8 flex flex-col justify-center items-center text-center">
              <svg className="w-12 h-12 md:w-16 md:h-16 text-[#1B342B]/10 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8V7a4 4 0 00-8 0v4h8z" /></svg>
              ...
              <Link href="/login" onClick={onClose} className="bg-[#1B342B] text-[#FDFBF7] px-8 py-3.5 rounded-sm hover:bg-[#A67C52] transition-colors text-[11px] font-semibold uppercase tracking-widest shadow-md">
                Sign In
              </Link>
            </div>
            ...
            <div className="h-full flex items-center justify-center">
              <div className="w-8 h-8 border-2 border-[#1B342B]/10 border-t-[#A67C52] rounded-full animate-spin"></div>
            </div>
            
          ) : cartItems.length === 0 ? (
            <div className="h-full p-8 flex flex-col justify-center items-center text-center">
               <svg className="w-16 h-16 md:w-20 md:h-20 text-[#1B342B]/10 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
               <p className="text-[#1B342B]/50 text-xs md:text-sm mb-8 leading-relaxed font-light px-4">Your shopping cart is currently empty.<br/>Explore our organic henna cones and sealants.</p>
               <button onClick={() => { onClose(); router.push('/shop'); }} className="border border-[#A67C52] text-[#A67C52] px-8 py-3.5 rounded-sm hover:bg-[#A67C52] hover:text-[#FDFBF7] transition-all duration-300 text-[11px] font-semibold uppercase tracking-widest">
                 Browse Boutique
               </button>
            </div>
          ) : (
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="flex gap-4 md:gap-5 items-center bg-white p-3 md:p-4 border border-[#1B342B]/5 shadow-sm hover:shadow-md transition-shadow duration-300">
                  <Link href={`/shop/${item.product?.id || '#'}`} onClick={onClose} className="relative w-20 h-24 md:w-24 md:h-28 bg-[#F4F1ED] shrink-0 block overflow-hidden rounded-sm">
                    <Image src={item.product?.image_url || '/placeholder.png'} alt={item.product?.name || 'Product'} fill className="object-cover hover:scale-105 transition-transform duration-700" />
                  </Link>
